...
%   stats  -  Struct with .spike and .burst fields (see above).
    ...
    % BH-FDR across the rate family (Q1: firing rate + burst rate).
    pRate = [stats.spike.pApprox, stats.burst.pApprox];
    [pRateAdj, rateRej] = bh_fdr(pRate, 0.05);
    stats.spike.pBH = pRateAdj(1);
    stats.spike.sigBH = rateRej(1);
    stats.burst.pBH = pRateAdj(2);
    stats.burst.sigBH = rateRej(2);

    fprintf('\n--- fig_stats_bootstrap (study=%s, n=%d boots) ---\n', study, opt.nBootstrap);
    print_bootstrap_line('Firing rate', stats.spike);
    print_bootstrap_line('Burst rate',  stats.burst);
    fprintf('  BH-FDR (rate family): spike p_BH=%.4f, burst p_BH=%.4f\n', ...
        stats.spike.pBH, stats.burst.pBH);
    fprintf('--------------------------------------------------\n');
    ...
    export_figure_stats(out, fullfile(statsDir, ...
