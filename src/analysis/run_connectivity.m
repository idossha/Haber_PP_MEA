...
%           PREPROCESS_AND_SAVE.
    ...
    if opt.save
        outDir = fullfile(cfg.paths.cache, 'connectivity');
    ...
    for k = 1:nPairs
        [bSpikes, tSpikes, bMeta, tMeta] = ...
            load_pair_spikes(pairs(k), opt.channels, cfg);
        ...
        % Resolve per-adjacency threshold: use explicit value if given,
        % otherwise pick the (1 - edgeDensity) percentile of the upper
        % triangle as a data-driven cutoff.
        bThresh = resolve_threshold(bRes.adjacency, opt);
        tThresh = resolve_threshold(tRes.adjacency, opt);

        bMetrics = network_metrics(bRes.adjacency, 'threshold', bThresh);
        tMetrics = network_metrics(tRes.adjacency, 'threshold', tThresh);

        results(k).pair      = pairs(k);
        results(k).baseline  = struct( ...
            'datasetName', bMeta.datasetName, ...
            'adjacency',   bRes.adjacency, ...
            'peakLagMs',   bRes.peakLagMs, ...
            'metrics',     bMetrics);
        results(k).treatment = struct( ...
            'datasetName', tMeta.datasetName, ...
            'adjacency',   tRes.adjacency, ...
            'peakLagMs',   tRes.peakLagMs, ...
            'metrics',     tMetrics);
        results(k).options = struct( ...
        ...
        if opt.save
            entry = results(k); %#ok<NASGU>
            outFile = fullfile(outDir, sprintf('connectivity_%s_%s.mat', ...
