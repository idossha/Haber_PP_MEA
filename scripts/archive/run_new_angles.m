...
%
%   Results are cached to output/shared/<study>_new_angles.mat
...
%           SPIKE_ENTROPY_SHANNON, AVALANCHES, PAIRED_STATS.
    ...
    %%% ----- cache path ---------------------------------------------------
    newAnglesDir = fullfile(cfg.paths.output, 'shared');
    if ~exist(newAnglesDir, 'dir')
        mkdir(newAnglesDir);
    end
    cachePath = fullfile(newAnglesDir, ...
